...
Replica.OnNew("PlayerData", function(replica)
	...
	if not player or not Players:FindFirstChild(player.Name) or player ~= Players.LocalPlayer then
		return
	end
				...
				scope:ShopPreview({
					...
					MouseEnter = function(x, y)
						TooltipText:set(
							("+" .. (item.Addition or 0))
								.. "/"
								.. (item.Multiplier and item.Multiplier + 1 or 1)
								.. "x"
						)
	...
	local gui = scope:New("ScreenGui")({
		...
		[Children] = {
			scope:Computed(function(use)
				if not use(TooltipVisible) then
					return nil
				end
			...
			scope:Computed(function(use)
				if not use(showMessageBox) then
					return nil
				end
			...
			scope:New("Frame")({
				...
				[Children] = {
					scope:ImageButton({
						ImageId = imageId,
						Size = UDim2.fromScale(0.025, 0.05),
						Position = UDim2.fromScale(0.09, 0.035),
						BG = bgColor,
						FG = textColor,
						ZIndex = 5,
					...
					scope:ImageButton({
						...
						Size = UDim2.fromScale(0.025, 0.05),
						Position = UDim2.fromScale(0.12, 0.035),
						BG = bgColor,
						FG = textColor,
						ZIndex = 5,
			...
			scope:TextButton({
				...
				Activated = function()
					...
					require(ReplicatedStorage.Utility.SFXPlayer).PlayAttached(
						"Click",
						game:GetService("SoundService"),
						{ SoundId = "rbxassetid://6895079853", Volume = 2, Pitch = 1.5 }
					)
